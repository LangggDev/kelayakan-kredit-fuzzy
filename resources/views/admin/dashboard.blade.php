...
@section('content')
                ...
                <div>
                    ...
                    @if($stats['total_analisis'] > 0)
                        <p class="text-xs text-slate-400 mt-1">
                            {{ round($stats['total_layak'] / $stats['total_analisis'] * 100) }}%
                            dari total</p>
                    @endif
                </div>
        ...
        <div class="stat-card p-5">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    @if($stats['total_analisis'] > 0)
                        <p class="text-xs text-slate-400 mt-1">
                            {{ round($stats['total_tidak_layak'] / $stats['total_analisis'] * 100) }}% dari total
                        </p>
                    @endif
                </div>
        ...
        <div class="lg:col-span-2 card p-6">
                ...
                @foreach($chartData as $data)
                    ...
                    <div class="flex-1 flex flex-col items-center gap-1">
                        <div class="w-full flex items-end gap-0.5" style="height:120px">
                            <div class="flex-1 rounded-t-md bg-indigo-500 transition-all" style="height:{{ $heightLayak }}%">
                            </div>
            ...
            <div class="flex items-center gap-4 mt-3">
                <span class="flex items-center gap-1.5 text-xs text-slate-500"><span
                        class="w-3 h-3 rounded bg-indigo-500"></span>Layak</span>
                <span class="flex items-center gap-1.5 text-xs text-slate-500"><span
                        class="w-3 h-3 rounded bg-red-400"></span>Tidak Layak</span>
            </div>
        </div>
        ...
        <div class="card p-6 flex flex-col gap-4">
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between py-2.5 border-b border-slate-50">
                    <span class="text-sm text-slate-500">Total debitur</span>
                ...
                <div class="flex items-center justify-between py-2.5">
                    ...
                    <span
                        class="text-xs font-semibold px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-full">Tsukamoto</span>
                </div>
            </div>
            <div class="mt-auto">
                <a href="{{ route('admin.analisis.index') }}"
                    class="btn-primary w-full py-2.5 rounded-xl text-white font-semibold text-sm text-center block">
    ...
    <div class="card overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
            ...
            <a href="{{ route('admin.analisis.index') }}" class="text-xs text-indigo-600 font-medium hover:underline">Lihat
                Semua</a>
        </div>
                ...
                <thead>
                    <tr class="bg-slate-50">
                        <th class="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                            Debitur</th>
                        <th class="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Analis
                        </th>
                        <th class="text-right px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Skor
                        </th>
                    ...
                    @forelse($recentAnalisis as $item)
                        <tr class="table-row">
                            ...
                            <td class="px-6 py-3.5 text-right">
                                <span
                                    class="font-mono font-semibold text-slate-800">{{ number_format($item->persentase_kelayakan, 1) }}</span>
                            </td>
                            ...
                            <td class="px-6 py-3.5 text-right text-slate-400 text-xs">{{ $item->created_at->format('d M Y') }}
                            </td>
                        </tr>
                    @empty
                        <tr>
