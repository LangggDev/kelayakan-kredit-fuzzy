...
@section('content')
    ...
    {{-- Filter --}}
    <div class="card p-4">
        <form action="{{ route('kepala_cabang.approval.index') }}" method="GET"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100" style="background:#f4f6fb">
                        <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">No
                        </th>
                    ...
                    @forelse($approvals as $item)
                        <tr class="table-row {{ $item->status_approval === 'menunggu' ? 'bg-amber-50/30' : '' }}">
                            ...
                            <td class="px-4 py-4 text-center">
                                <span
                                    class="font-mono font-bold text-slate-800">{{ number_format($item->persentase_kelayakan, 1) }}</span>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <span
                            ...
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('kepala_cabang.approval.show', $item->id) }}"
                                    ...
                                    <i
                                        class="fa-solid {{ $item->status_approval === 'menunggu' ? 'fa-signature' : ($item->status_approval === 'tidak_layak' ? 'fa-eye' : 'fa-stamp') }}"></i>
