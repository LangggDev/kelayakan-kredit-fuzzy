...
@section('content')
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="text-center px-4 py-3.5 text-xs font-semibold text-slate-500 uppercase">Aksi</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($rules as $rule)
                        @php
                            $himpunanMap = [
                                'character' => [
                                    'baik' => 'S1 (tipe Slik 1)',
                                    'cukup' => 'S2 (tipe Slik 2)',
                                    'buruk' => 'S3 (tipe Slik 3)',
                                ],
                                'capacity' => [
                                    'tinggi' => 'Sangat Layak',
                                    'sedang' => 'Layak',
                            ...
                            $colorMap = [
                                'S1 (tipe Slik 1)' => 'bg-blue-50 text-blue-700',
                                'S2 (tipe Slik 2)' => 'bg-yellow-50 text-yellow-700',
                                'S3 (tipe Slik 3)' => 'bg-red-50 text-red-700',
                        ...
                        <tr class="table-row">
                            <td class="px-4 py-3.5 text-center">
                                <span
                                    class="w-7 h-7 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold flex items-center justify-center mx-auto">{{ $rule->nomor_rule }}</span>
                            </td>
                            @foreach(['character', 'capacity', 'capital', 'collateral', 'condition'] as $field)
                                @php
                                    $val = $rule->$field;
                                    $displayVal = $himpunanMap[$field][$val] ?? ucfirst($val);
                                @endphp
                                <td class="px-4 py-3.5 text-center">
                                    <span
                                        class="px-2 py-0.5 rounded-full text-xs font-semibold {{ $colorMap[$displayVal] ?? 'bg-slate-100 text-slate-600' }}">{{ $displayVal }}</span>
                                </td>
                            @endforeach
                            <td class="px-4 py-3.5 text-center">
                                <span
                                    class="px-2.5 py-1 rounded-full text-xs font-bold {{ $rule->kelayakan === 'layak' ? 'badge-layak' : 'badge-tidak-layak' }}">
                                    {{ $rule->kelayakan === 'layak' ? 'LAYAK' : 'TDK LAYAK' }}
                                </span>
                            </td>
                            <!-- <td class="px-4 py-3.5 text-center text-xs text-slate-500 capitalize">
                                                        {{ str_replace('_', ' ', $rule->output_tipe) }}
                                                    </td> -->
                            <td class="px-4 py-3.5 text-center">
                                <span
                                    class="w-2 h-2 rounded-full inline-block {{ $rule->is_active ? 'bg-green-500' : 'bg-slate-300' }}"></span>
                            </td>
                            <td class="px-4 py-3.5 text-right">
                                <div class="flex items-center gap-2 justify-end">
                                    ...
                                    <form action="{{ route('admin.rules.destroy', $rule->id) }}" method="POST"
                                        onsubmit="return confirm('Hapus rule #' + {{ $rule->nomor_rule }} + '?')">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-14 text-center text-slate-400"><i
                                    class="fa-solid fa-code-branch text-2xl mb-2 block"></i>Belum ada rule fuzzy</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="card p-4">
        ...
        <div class="flex flex-wrap gap-2">
            <span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700">S1 (tipe Slik 1) / Layak</span>
            <span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-yellow-50 text-yellow-700">S2 (tipe Slik 2)</span>
            <span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-red-50 text-red-700">S3 (tipe Slik 3) / Tidak Layak</span>
