...
@section('content')
    ...
    <div
        class="rounded-2xl overflow-hidden {{ $hasilAnalisis->keputusan === 'Layak' ? 'bg-gradient-to-r from-green-500 to-emerald-600' : 'bg-gradient-to-r from-red-500 to-rose-600' }} p-6 text-white">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <div class="w-16 h-16 rounded-2xl bg-white/20 flex items-center justify-center flex-shrink-0">
                    <i
                        class="fa-solid {{ $hasilAnalisis->keputusan === 'Layak' ? 'fa-circle-check' : 'fa-circle-xmark' }} text-3xl"></i>
                </div>
                <div>
                    ...
                    <p class="text-white/70 text-sm mt-1">Skor: <span
                            class="text-white font-semibold font-mono">{{ number_format($hasilAnalisis->nilai_defuzzifikasi, 4) }}</span>
                        &mdash; {{ number_format($hasilAnalisis->persentase_kelayakan, 2) }}%</p>
                </div>
            </div>
        ...
        <div class="card p-6">
            ...
            <dl class="space-y-2">
                <div class="flex justify-between py-2 border-b border-slate-50">
                    ...
                    <dd class="text-sm font-semibold">Rp {{ number_format($hasilAnalisis->penghasilan, 0, ',', '.') }}</dd>
                </div>
                <div class="flex justify-between py-2 border-b border-slate-50">
                    <dt class="text-sm text-slate-500">Jumlah Pinjaman</dt>
                    <dd class="text-sm font-semibold">Rp {{ number_format($hasilAnalisis->jumlah_pinjaman, 0, ',', '.') }}</dd>
                </div>
                <div class="flex justify-between py-2 border-b border-slate-50">
                    <dt class="text-sm text-slate-500">Rasio Cicilan</dt>
                    <dd class="text-sm font-semibold">{{ number_format($hasilAnalisis->rasio_cicilan, 2) }}%</dd>
                </div>
                <div class="flex justify-between py-2 border-b border-slate-50">
                    <dt class="text-sm text-slate-500">Jangka Waktu</dt>
                    <dd class="text-sm font-semibold">{{ $hasilAnalisis->jangka_waktu }} bulan</dd>
                </div>
                <div class="flex justify-between py-2 border-b border-slate-50">
                    <dt class="text-sm text-slate-500">Analis</dt>
                    <dd class="text-sm font-semibold">{{ $hasilAnalisis->user?->name ?? 'Analis Terhapus' }}</dd>
                </div>
                <div class="flex justify-between py-2">
                    <dt class="text-sm text-slate-500">Tanggal</dt>
                    <dd class="text-sm font-semibold">
                        {{ $hasilAnalisis->created_at ? $hasilAnalisis->created_at->format('d M Y, H:i') : '-' }}</dd>
                </div>
            </dl>
        </div>
    </div>
    ...
    @if(!empty($hasilAnalisis->detail_rule))
        ...
        <div class="card p-6">
            ...
            @php $sumAZ = array_sum(array_column($hasilAnalisis->detail_rule, 'alpha_z'));
            $sumA = array_sum(array_column($hasilAnalisis->detail_rule, 'alpha')); @endphp
            <div class="bg-slate-50 rounded-xl p-5 font-mono text-sm">
                <div class="mb-2">z* = {{ number_format($sumAZ, 4) }} / {{ number_format($sumA, 4) }}</div>
                <div class="font-bold text-lg text-indigo-700">z* = {{ number_format($hasilAnalisis->nilai_defuzzifikasi, 4) }}
                    <span class="text-slate-500 text-base font-normal ml-2">≈
                        {{ number_format($hasilAnalisis->persentase_kelayakan, 2) }}%</span></div>
            </div>
            <div class="mt-4">
                <div class="relative h-4 bg-gradient-to-r from-red-300 via-yellow-300 to-green-400 rounded-full">
                    ...
                    <div class="absolute top-1/2 -translate-y-1/2 w-5 h-5 rounded-full border-2 border-white shadow {{ $hasilAnalisis->keputusan === 'Layak' ? 'bg-green-600' : 'bg-red-500' }}"
                        style="left:calc({{ min(max($hasilAnalisis->persentase_kelayakan, 0), 100) }}% - 10px)"></div>
                </div>
                <p class="text-center text-sm text-slate-600 mt-3">Skor <strong
                        class="font-mono">{{ number_format($hasilAnalisis->persentase_kelayakan, 2) }}</strong> → <strong
                        class="{{ $hasilAnalisis->keputusan === 'Layak' ? 'text-green-600' : 'text-red-600' }}">{{ $hasilAnalisis->keputusan }}</strong>
                </p>
            </div>
        </div>
    @endif
