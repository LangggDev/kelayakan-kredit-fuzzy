    ...
    <form action="{{ route('admin.rules.store') }}" method="POST">
        ...
        <div class="card p-6 space-y-5">
            <div class="flex items-center gap-3 pb-3 border-b border-slate-100">
                ...
                <span class="text-xs px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-700 font-semibold">Rule #{{ $nextNo }}</span>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    $ante = [
                        ...
                        'capacity'   => ['C2 — Capacity',   ['tinggi','sedang','rendah'],'green'],
                        'capital'    => ['C3 — Capital',    ['besar','sedang','kecil'], 'amber'],
                        'collateral' => ['C4 — Collateral', ['tinggi','sedang','rendah'],'purple'],
                        'condition'  => ['C5 — Condition',  ['baik','cukup','buruk'],  'rose'],
                    ];
                    ...
                    <div>
                        ...
                        <select name="{{ $field }}" class="w-full px-3 py-2.5 border {{ $borderMap[$color] }} rounded-xl text-sm bg-slate-50 capitalize" required>
                            @foreach($opts as $opt)
            ...
            <div class="pt-4 border-t border-slate-100">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <select name="kelayakan" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50" required>
                            <option value="layak" {{ old('kelayakan')==='layak'?'selected':'' }}>Layak</option>
                            <option value="tidak_layak" {{ old('kelayakan')==='tidak_layak'?'selected':'' }}>Tidak Layak</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase">Tipe Output</label>
                        <select name="output_tipe" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50" required>
                            <option value="linear_naik" {{ old('output_tipe')==='linear_naik'?'selected':'' }}>Linear Naik (Layak)</option>
                            <option value="linear_turun" {{ old('output_tipe')==='linear_turun'?'selected':'' }}>Linear Turun (Tdk Layak)</option>
                        </select>
                    </div>
                    <div>
            ...
            <div>
                ...
                <input type="text" name="deskripsi" value="{{ old('deskripsi') }}" placeholder="Deskripsi singkat rule..."
