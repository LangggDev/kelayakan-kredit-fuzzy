            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="text-left px-4 py-3.5 text-xs font-semibold text-slate-500 uppercase">Calon Debitur</th>
                    ...
                    @forelse($hasilAnalisis as $item)
                        <tr class="table-row">
                            ...
                            @foreach(['skor_character' => 'blue', 'skor_capacity' => 'green', 'skor_capital' => 'amber', 'skor_collateral' => 'purple', 'skor_condition' => 'rose'] as $col => $color)
                                ...
                                <td class="px-2 py-3.5 text-center">
                                    <div class="flex flex-col items-center gap-0.5">
                                        <span
                                            class="text-xs font-mono font-semibold text-slate-700">{{ number_format($skor, 0) }}</span>
                                        <div class="w-8 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                            <div class="h-full rounded-full bg-{{ $color }}-400"
                                                style="width:{{ min($skor, 100) }}%"></div>
                                        </div>
                                    </div>
                                </td>
                            @endforeach
                            ...
                            <td class="px-4 py-3.5 text-center">
                                <span
                                    class="font-mono font-bold text-slate-800 text-sm">{{ number_format($item->persentase_kelayakan, 1) }}</span>
                            </td>
                            {{-- Keputusan --}}
                            <td class="px-4 py-3.5 text-center">
                                <span
                                    class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold {{ $item->keputusan === 'Layak' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600' }}">
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-1.5 justify-end">
                                    ...
                                    <a href="{{ route('admin.analisis.show', $item->id) }}"
                                        class="p-1.5 rounded-lg bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition-colors"
                                        title="Detail">
