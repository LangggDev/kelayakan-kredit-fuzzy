...
@section('page-subtitle', $analisis->calonNasabah->nama . ' — ' . ($analisis->created_at ? $analisis->created_at->format('d M Y') : '-'))
    ...
    <div class="rounded-2xl p-5 text-white" style="background:linear-gradient(135deg,#1a2e5a,#2d4190)">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0"
                ...
                <i
                    class="fa-solid {{ $analisis->keputusan === 'Layak' ? 'fa-circle-check text-green-400' : 'fa-circle-xmark text-red-400' }} text-2xl"></i>
            </div>
            <div>
        ...
        <div class="card p-5">
            ...
            <dl class="space-y-2">
                <div class="flex justify-between py-1.5 border-b border-slate-50">
                    <dt class="text-xs text-slate-500 flex items-center gap-1"><span
                            class="w-4 h-4 rounded text-white flex items-center justify-center font-bold"
                            style="background:#2563eb;font-size:8px">C1</span>Skor Kredit</dt>
                    <dd class="text-xs font-semibold">{{ number_format($analisis->skor_kredit, 0) }} / 100</dd>
                </div>
                <div class="flex justify-between py-1.5 border-b border-slate-50">
                    <dt class="text-xs text-slate-500 flex items-center gap-1"><span
                            class="w-4 h-4 rounded text-white flex items-center justify-center font-bold"
                            style="background:#16a34a;font-size:8px">C2</span>Penghasilan/Bln</dt>
                ...
                <div class="flex justify-between py-1.5 border-b border-slate-50">
                    <dt class="text-xs text-slate-500 pl-5">Rasio Cicilan</dt>
                    <dd
                ...
                <div class="flex justify-between py-1.5 border-b border-slate-50">
                    <dt class="text-xs text-slate-500 flex items-center gap-1"><span
                            class="w-4 h-4 rounded text-white flex items-center justify-center font-bold"
                            style="background:#d97706;font-size:8px">C3</span>Aset Bersih</dt>
                    <dd class="text-xs font-semibold">Rp {{ number_format($analisis->aset_bersih, 0, ',', '.') }}</dd>
                </div>
                <div class="flex justify-between py-1.5 border-b border-slate-50">
                    <dt class="text-xs text-slate-500 flex items-center gap-1"><span
                            class="w-4 h-4 rounded text-white flex items-center justify-center font-bold"
                            style="background:#9333ea;font-size:8px">C4</span>Nilai Agunan</dt>
                    <dd class="text-xs font-semibold">Rp {{ number_format($analisis->nilai_agunan, 0, ',', '.') }}</dd>
                </div>
                <div class="flex justify-between py-1.5">
                    <dt class="text-xs text-slate-500 flex items-center gap-1"><span
                            class="w-4 h-4 rounded text-white flex items-center justify-center font-bold"
                            style="background:#e11d48;font-size:8px">C5</span>Kondisi Ekonomi</dt>
                    <dd class="text-xs font-semibold">{{ number_format($analisis->kondisi_ekonomi, 0) }} / 100</dd>
                </div>
            </dl>
        </div>
    </div>
