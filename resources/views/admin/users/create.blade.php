    ...
    <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-5">
        ...
        <div class="card p-6">
            ...
            <div class="grid grid-cols-3 gap-3">
                @php
                $roles = [
                    'analis'        => ['Kredit Analis',  'fa-user-pen',  '#2563eb', 'Melakukan analisis kelayakan kredit'],
                    'kepala_cabang' => ['Kepala Cabang',  'fa-user-tie',  '#d97706', 'Menyetujui hasil analisis kredit'],
                    'marketing'     => ['Marketing',      'fa-bullhorn',  '#16a34a', 'Melihat data analisis yang disetujui'],
                ];
                @endphp
                @foreach($roles as $key => [$label, $icon, $color, $desc])
                <label class="cursor-pointer border-2 border-slate-200 rounded-xl p-3 hover:border-slate-300 transition-all has-[:checked]:border-blue-400 has-[:checked]:shadow-sm">
                    <input type="radio" name="role" value="{{ $key }}" class="sr-only"
                        {{ old('role') === $key ? 'checked' : '' }} onchange="updateRoleFields('{{ $key }}')">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center mb-2" style="background:{{ $color }}20">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="col-span-2">
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">
                        ...
                        <span class="ml-2 px-2 py-0.5 rounded-full text-xs font-semibold" style="background:#eef1f8; color:#1a2e5a">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="nik" value="{{ old('nik') }}"
                            placeholder="NIK unik pengguna (16 karakter)"
                            maxlength="16"
                            minlength="16"
                ...
                <div class="col-span-2 flex items-center gap-2">
                    <input type="checkbox" name="is_active" value="1" id="isActive" checked class="rounded border-slate-300 w-4 h-4" style="accent-color:#1a2e5a">
        ...
        <div class="card p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="col-span-2 role-field field-analis hidden">
                    ...
                    <input type="text" name="jabatan" value="{{ old('jabatan') }}" placeholder="e.g. Kredit Analis Senior"
                        class="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50 focus:bg-white">
                </div>
                ...
                <div class="col-span-2 role-field field-kepala_cabang hidden">
                    ...
                    <input type="text" name="cabang" value="{{ old('cabang') }}" placeholder="e.g. Cabang Jakarta Selatan"
                        class="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50 focus:bg-white">
                </div>
        ...
        <div class="flex gap-3">
            <button type="submit" class="btn-primary px-6 py-2.5 rounded-xl text-white font-bold text-sm flex items-center gap-2">
