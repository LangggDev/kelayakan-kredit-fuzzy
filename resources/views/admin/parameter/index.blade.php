    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-2">
        <div class="flex items-center gap-2 flex-wrap">
            @foreach($meta as $key => $m)
                ...
                <a href="#{{ $key }}"
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold {{ $c['himpunan_bg'] }} hover:opacity-80 transition-opacity">
                    <i class="fa-solid {{ $m['icon'] }} text-xs"></i> {{ $m['kode'] }}
                </a>
            @endforeach
        </div>
        <a href="{{ route('admin.parameter.create') }}"
            class="btn-primary flex items-center gap-2 px-5 py-2.5 rounded-xl text-white font-semibold text-sm flex-shrink-0">
            ...
            <div class="card overflow-hidden" id="{{ $key }}">
                ...
                <div class="bg-gradient-to-r {{ $c['header'] }} border-b {{ $c['border'] }} px-6 py-4">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-xl {{ $c['badge'] }} flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                                {{ $m['kode'] }}
                            </div>
                            <div>
                        ...
                        <div class="text-right flex-shrink-0">
                            <div class="text-xs text-slate-400 mb-1">Satuan: <span
                                    class="font-medium text-slate-600">{{ $m['satuan'] }}</span></div>
                            <div class="text-xs {{ $c['himpunan_bg'] }} px-2 py-0.5 rounded-full inline-block">
                                {{ $m['panduan'] }}</div>
                        </div>
                    </div>
                </div>
                ...
                @if($paramList->count() > 0)
                    ...
                    <div class="px-6 pt-5 pb-2">
                        <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Visualisasi Fungsi Keanggotaan
                        </p>
                            ...
                            @foreach($paramList as $param)
                                ...
                                <div class="{{ $c['bg'] }} rounded-xl p-4 border {{ $c['border'] }}">
                                    <div class="flex items-center justify-between mb-3">
                                        ...
                                        <span
                                            class="flex items-center gap-1 {{ $tf['color'] }} px-2 py-0.5 rounded-full text-xs font-medium">
                                    ...
                                    @php
                                        $a = (float) $param->a;
                                        $b = (float) $param->b;
                                        $c_val = $param->c ? (float) $param->c : null;
                                        $maxVal = $c_val ?? $b;
                                        $minVal = $a;
                                        $range = $maxVal - $minVal;
                                        if ($range <= 0)
                                    ...
                                    <div class="mb-3 bg-white rounded-lg p-2">
                                        <svg viewBox="0 0 100 50" class="w-full h-10" preserveAspectRatio="none">
                                            <polyline points="{{ $points }}" fill="none"
                                                stroke="{{ ['blue' => '#3b82f6', 'green' => '#22c55e', 'amber' => '#f59e0b', 'purple' => '#a855f7', 'rose' => '#f43f5e'][$m['color']] }}"
                                                stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
                                            <polyline points="{{ $points }} 100,50 0,50"
                                                fill="{{ ['blue' => '#dbeafe', 'green' => '#dcfce7', 'amber' => '#fef3c7', 'purple' => '#f3e8ff', 'rose' => '#ffe4e6'][$m['color']] }}"
                                    ...
                                    <div class="flex gap-2 mt-3 pt-2 border-t {{ $c['border'] }}">
                                        ...
                                        <form action="{{ route('admin.parameter.destroy', $param->id) }}" method="POST"
                                            ...
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                class="px-3 py-1.5 rounded-lg bg-white border border-red-200 text-red-500 hover:bg-red-50 text-xs font-medium transition-colors">
                    ...
                    <div class="px-6 pb-5 mt-3">
                        <details class="group">
                            <summary
                                class="text-xs font-semibold text-slate-400 cursor-pointer hover:text-slate-600 flex items-center gap-2 select-none">
                                ...
                                <table class="w-full text-xs">
                                    <thead>
                                        <tr class="bg-slate-50">
                                            <th class="text-left px-4 py-2.5 font-semibold text-slate-500 uppercase tracking-wider">
                                                Himpunan</th>
                                            <th class="text-left px-4 py-2.5 font-semibold text-slate-500 uppercase tracking-wider">
                                                Tipe Fungsi</th>
                                            <th
                                                class="text-center px-4 py-2.5 font-semibold text-slate-500 uppercase tracking-wider">
                                                a</th>
                                            <th
                                                class="text-center px-4 py-2.5 font-semibold text-slate-500 uppercase tracking-wider">
                                                b</th>
                                        ...
                                        @foreach($paramList as $param)
                                            <tr class="table-row">
                                                <td class="px-4 py-2.5">
                                                    <span
                                                        class="{{ $c['himpunan_bg'] }} px-2 py-0.5 rounded-full text-xs font-semibold capitalize">{{ $param->himpunan }}</span>
                                                </td>
                                                <td class="px-4 py-2.5 text-slate-600">
                                                    {{ str_replace('_', ' ', ucfirst($param->tipe_fungsi)) }}</td>
                                                <td class="px-4 py-2.5 text-center font-mono text-slate-700">
                                                    {{ number_format($param->a, 2, '.', '') }}</td>
                                                <td class="px-4 py-2.5 text-center font-mono text-slate-700">
                                                    {{ number_format($param->b, 2, '.', '') }}</td>
                                                <td class="px-4 py-2.5 text-center font-mono text-slate-400">
                                                    {{ $param->c !== null ? number_format($param->c, 2, '.', '') : '—' }}</td>
                                                <td class="px-4 py-2.5 text-center">
                                                    <span
                                                        class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold {{ $param->is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500' }}">
                                                        <span
                                                            class="w-1.5 h-1.5 rounded-full {{ $param->is_active ? 'bg-green-600' : 'bg-slate-400' }}"></span>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-slate-600">
                    <div class="flex items-start gap-2">
                        <span
                            class="flex items-center gap-1 text-green-600 bg-green-50 px-2 py-0.5 rounded-full font-medium flex-shrink-0"><i
                                class="fa-solid fa-arrow-trend-up text-xs"></i> Linear Naik</span>
                        <span>Nilai meningkat dari a (μ=0) ke b (μ=1). Digunakan untuk himpunan "tinggi/baik/besar".</span>
                    </div>
                    <div class="flex items-start gap-2">
                        <span
                            class="flex items-center gap-1 text-red-500 bg-red-50 px-2 py-0.5 rounded-full font-medium flex-shrink-0"><i
                                class="fa-solid fa-arrow-trend-down text-xs"></i> Linear Turun</span>
                        <span>Nilai menurun dari a (μ=1) ke b (μ=0). Digunakan untuk himpunan "rendah/buruk/kecil".</span>
                    </div>
                    <div class="flex items-start gap-2">
                        <span
                            class="flex items-center gap-1 text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full font-medium flex-shrink-0"><i
                                class="fa-solid fa-caret-up text-xs"></i> Segitiga</span>
