    ...
    <div class="card overflow-hidden">
        ...
        <div class="divide-y divide-slate-50">
            ...
            <div class="flex items-center gap-3 px-5 py-3.5 hover:bg-slate-50 transition-colors">
                <div class="w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 text-white" style="background:#1a2e5a">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-slate-400">Analis: {{ $item->user->name }} &bull; {{ $item->created_at->diffForHumans() }}</div>
                </div>
                <div class="flex items-center gap-2 flex-shrink-0">
                    <span class="text-xs font-semibold {{ $item->keputusan === 'Layak' ? 'badge-layak' : 'badge-tidak-layak' }} px-2 py-0.5 rounded-full">
                        {{ $item->keputusan }}
                    </span>
        ...
        <div class="px-5 py-3 border-t border-slate-100 text-center">
            <a href="{{ route('kepala_cabang.approval.index', ['status'=>'menunggu']) }}" class="text-xs font-semibold hover:underline" style="color:#1a2e5a">
    ...
    <div class="card overflow-hidden">
        ...
        <div class="divide-y divide-slate-50">
            ...
            <div class="flex items-center gap-3 px-5 py-3.5">
                ...
                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border {{ $item->status_color }}">
                    {{ $item->status_label }}
                </span>
            </div>
            @empty
