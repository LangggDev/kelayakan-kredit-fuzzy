...
<div class="card p-4">
    <form action="{{ route('admin.users.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3 flex-wrap">
        ...
        <select name="role" class="px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50">
            ...
            <option value="analis"        {{ request('role')==='analis'        ?'selected':'' }}>Kredit Analis</option>
            <option value="kepala_cabang" {{ request('role')==='kepala_cabang' ?'selected':'' }}>Kepala Cabang</option>
        ...
        <a href="{{ route('admin.users.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-medium text-sm hover:bg-slate-50 flex items-center gap-2">
            <i class="fa-solid fa-xmark"></i> Reset
        </a>
        @endif
        <a href="{{ route('admin.users.create') }}" class="btn-primary px-5 py-2.5 rounded-xl text-white font-semibold text-sm flex items-center gap-2">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-100" style="background:#f4f6fb">
                    ...
                    <th class="text-left px-4 py-3.5 text-xs font-semibold text-slate-500 uppercase">NIP</th>
                ...
                <tr class="table-row">
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style="background:#1a2e5a">
                    ...
                    <td class="px-4 py-4">
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold {{ $rc['color'] }}">
                    ...
                    <td class="px-4 py-4 text-center">
                        <form action="{{ route('admin.users.toggle-status', $user->id) }}" method="POST">
                            @csrf @method('PATCH')
                            <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold transition-colors
