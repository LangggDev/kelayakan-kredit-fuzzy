...
<div class="rounded-2xl p-5 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
    ...
    <div class="flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background:rgba(245,166,35,0.2)">
            <i class="fa-solid {{ $analisis->keputusan === 'Layak' ? 'fa-circle-check text-green-400' : 'fa-circle-xmark text-red-400' }} text-xl"></i>
        </div>
        <div>
...
{{-- Read only notice --}}
<div class="flex items-center gap-2 px-4 py-3 rounded-xl text-xs" style="background:#eef1f8; color:#1a2e5a; border:1px solid #d5ddef">
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    ...
    <div class="card p-5">
        ...
        <dl class="space-y-2">
            <div class="flex justify-between py-1.5 border-b border-slate-50"><dt class="text-xs text-slate-500">Penghasilan/Bulan</dt><dd class="text-xs font-semibold">Rp {{ number_format($analisis->penghasilan, 0, ',', '.') }}</dd></div>
            <div class="flex justify-between py-1.5 border-b border-slate-50"><dt class="text-xs text-slate-500">Jumlah Pinjaman</dt><dd class="text-xs font-semibold">Rp {{ number_format($analisis->jumlah_pinjaman, 0, ',', '.') }}</dd></div>
            <div class="flex justify-between py-1.5 border-b border-slate-50"><dt class="text-xs text-slate-500">Jangka Waktu</dt><dd class="text-xs font-semibold">{{ $analisis->jangka_waktu }} bulan</dd></div>
            <div class="flex justify-between py-1.5 border-b border-slate-50"><dt class="text-xs text-slate-500">Rasio Cicilan</dt><dd class="text-xs font-semibold {{ $analisis->rasio_cicilan <= 30 ? 'text-green-600' : ($analisis->rasio_cicilan <= 50 ? 'text-amber-600' : 'text-red-600') }}">{{ number_format($analisis->rasio_cicilan, 2) }}%</dd></div>
            <div class="flex justify-between py-1.5 border-b border-slate-50"><dt class="text-xs text-slate-500">Nilai Agunan</dt><dd class="text-xs font-semibold">Rp {{ number_format($analisis->nilai_agunan, 0, ',', '.') }}</dd></div>
            <div class="flex justify-between py-1.5 border-b border-slate-50"><dt class="text-xs text-slate-500">Kredit Analis</dt><dd class="text-xs font-semibold">{{ $analisis->user->name }}</dd></div>
            <div class="flex justify-between py-1.5"><dt class="text-xs text-slate-500">Disetujui KC</dt><dd class="text-xs font-semibold text-green-600">{{ $analisis->approvedBy?->name }} &bull; {{ $analisis->approved_at?->format('d M Y') }}</dd></div>
        </dl>
    </div>
</div>
