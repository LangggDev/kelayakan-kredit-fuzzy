...
@section('content')
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="stat-card p-5">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-xs font-medium text-slate-400 uppercase tracking-wider">Debitur</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="card p-6">
            ...
            <a href="{{ route('analis.analisis.create') }}"
                class="btn-primary flex items-center gap-3 p-4 rounded-xl text-white group mb-3">
                <div
                    class="w-10 h-10 rounded-lg bg-white/20 flex items-center justify-center group-hover:bg-white/30 flex-shrink-0">
                    <i class="fa-solid fa-plus text-white"></i>
                </div>
                <div>
                    <div class="font-semibold text-sm">Analisis Baru</div>
                    <div class="text-xs text-white/70">Input data debitur untuk dianalisis</div>
                </div>
            </a>
            <a href="{{ route('analis.analisis.index') }}"
                class="flex items-center gap-3 p-4 rounded-xl border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 transition-all group">
                <div
                    class="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center group-hover:bg-indigo-100 flex-shrink-0">
                ...
                <div class="mt-5 pt-4 border-t border-slate-100">
                    ...
                    <div class="progress-bar mb-1.5">
                        <div class="progress-fill"
                            style="width:{{ $stats['total_analisis'] > 0 ? round($stats['total_layak'] / $stats['total_analisis'] * 100) : 0 }}%">
                        </div>
                    </div>
        ...
        <div class="lg:col-span-2 card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
                ...
                <a href="{{ route('analis.analisis.index') }}"
                    class="text-xs text-indigo-600 font-medium hover:underline">Lihat Semua</a>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($recentAnalisis as $item)
                            <tr class="table-row cursor-pointer"
                                onclick="window.location='{{ route('analis.analisis.show', $item->id) }}'">
                                ...
                                <td class="px-6 py-3.5 text-right text-slate-400 text-xs">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-slate-400">
                                    ...
                                    Belum ada analisis. <a href="{{ route('analis.analisis.create') }}"
                                        class="text-indigo-600 hover:underline">Mulai analisis</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
